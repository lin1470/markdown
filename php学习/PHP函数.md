...
1.**全局作用域**：用户自定义全局变量，但是是在函数外部定义的变量。全局作用域的变量能够在**函数外部**的任何地方使用，其生命周期是**从定义直到脚本执行结束**（手动释放除外）    
